...
#define LIBRARY_CLASS_ZAPHIE "zp50_class_zaphie"
...
public plugin_init()
{
	register_plugin("[ZP] Zaphie Mod Flashlight", PLUGIN_VERSION, "mostten");
	
	info_target = engfunc(EngFunc_AllocString, "info_target");
	
	g_varFlashlight = register_cvar("zaphie_mod_flashlight", "1");
	g_varShowAllMod = register_cvar("zaphie_mod_flashlight_show_all", "0");
...
{
	if (equal(module, LIBRARY_ZP50_CORE) || equal(module, LIBRARY_ZP50_GAMEMODES) || equal(module, LIBRARY_FLASHLIGHT) || equal(module, LIBRARY_CLASS_ZAPHIE))
...
{
	g_bIsZaphieMod = (LibraryExists(LIBRARY_CLASS_ZAPHIE, LibType_Library) && game_mode_id == zp_gamemodes_get_id("Zaphie Mode"));
	set_all_cone_draw(true);
}

public zp_fw_gamemodes_end()
{
	g_bIsZaphieMod = false;
...
{
	new entity = engfunc(EngFunc_CreateNamedEntity, info_target);
...
set_cone_draw(entity, bool:draw)
{
	...
	{
		set_pev(entity, pev_effects, (get_pcvar_num(g_varShowAllMod) || g_bIsZaphieMod)?0:EF_NODRAW);
	}
	else
	{
...
{
	if(zp_class_zaphie_get(client) || zp_core_is_zombie(client))
		return false;
	return true;
}
