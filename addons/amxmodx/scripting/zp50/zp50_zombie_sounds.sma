...
// Ghost sounds
new Array:g_sound_ghost_pain
new Array:g_sound_ghost_die
new Array:g_sound_ghost_fall
new Array:g_sound_ghost_miss_slash
new Array:g_sound_ghost_miss_wall
new Array:g_sound_ghost_hit_normal
new Array:g_sound_ghost_hit_stab
new Array:g_sound_ghost_idle
new Array:g_sound_ghost_idle_last
...
public plugin_precache()
{
	...
	g_sound_ghost_pain = ArrayCreate(SOUND_MAX_LENGTH, 1)
	g_sound_ghost_die = ArrayCreate(SOUND_MAX_LENGTH, 1)
	g_sound_ghost_fall = ArrayCreate(SOUND_MAX_LENGTH, 1)
	g_sound_ghost_miss_slash = ArrayCreate(SOUND_MAX_LENGTH, 1)
	g_sound_ghost_miss_wall = ArrayCreate(SOUND_MAX_LENGTH, 1)
	g_sound_ghost_hit_normal = ArrayCreate(SOUND_MAX_LENGTH, 1)
	g_sound_ghost_hit_stab = ArrayCreate(SOUND_MAX_LENGTH, 1)
	g_sound_ghost_idle = ArrayCreate(SOUND_MAX_LENGTH, 1)
	g_sound_ghost_idle_last = ArrayCreate(SOUND_MAX_LENGTH, 1)
	...
	amx_load_setting_string_arr(ZP_GHOST_SETTINGS_FILE, "Sounds", "GHOST PAIN", g_sound_ghost_pain)
	amx_load_setting_string_arr(ZP_GHOST_SETTINGS_FILE, "Sounds", "GHOST DIE", g_sound_ghost_die)
	amx_load_setting_string_arr(ZP_GHOST_SETTINGS_FILE, "Sounds", "GHOST FALL", g_sound_ghost_fall)
	amx_load_setting_string_arr(ZP_GHOST_SETTINGS_FILE, "Sounds", "GHOST MISS SLASH", g_sound_ghost_miss_slash)
	amx_load_setting_string_arr(ZP_GHOST_SETTINGS_FILE, "Sounds", "GHOST MISS WALL", g_sound_ghost_miss_wall)
	amx_load_setting_string_arr(ZP_GHOST_SETTINGS_FILE, "Sounds", "GHOST HIT NORMAL", g_sound_ghost_hit_normal)
	amx_load_setting_string_arr(ZP_GHOST_SETTINGS_FILE, "Sounds", "GHOST HIT STAB", g_sound_ghost_hit_stab)
	amx_load_setting_string_arr(ZP_GHOST_SETTINGS_FILE, "Sounds", "GHOST IDLE", g_sound_ghost_idle)
	amx_load_setting_string_arr(ZP_GHOST_SETTINGS_FILE, "Sounds", "GHOST IDLE LAST", g_sound_ghost_idle_last)
	...
	// Ghost sounds
	if (ArraySize(g_sound_ghost_pain) == 0)
	{
		for (index = 0; index < sizeof sound_ghost_pain; index++)
			ArrayPushString(g_sound_ghost_pain, sound_ghost_pain[index])
		
		// Save to external file
		amx_save_setting_string_arr(ZP_GHOST_SETTINGS_FILE, "Sounds", "GHOST PAIN", g_sound_ghost_pain)
	}
	if (ArraySize(g_sound_ghost_die) == 0)
	{
		for (index = 0; index < sizeof sound_ghost_die; index++)
			ArrayPushString(g_sound_ghost_die, sound_ghost_die[index])
		
		// Save to external file
		amx_save_setting_string_arr(ZP_GHOST_SETTINGS_FILE, "Sounds", "GHOST DIE", g_sound_ghost_die)
	}
	if (ArraySize(g_sound_ghost_fall) == 0)
	{
		for (index = 0; index < sizeof sound_ghost_fall; index++)
			ArrayPushString(g_sound_ghost_fall, sound_ghost_fall[index])
		
		// Save to external file
		amx_save_setting_string_arr(ZP_GHOST_SETTINGS_FILE, "Sounds", "GHOST FALL", g_sound_ghost_fall)
	}
	if (ArraySize(g_sound_ghost_miss_slash) == 0)
	{
		for (index = 0; index < sizeof sound_ghost_miss_slash; index++)
			ArrayPushString(g_sound_ghost_miss_slash, sound_ghost_miss_slash[index])
		
		// Save to external file
		amx_save_setting_string_arr(ZP_GHOST_SETTINGS_FILE, "Sounds", "GHOST MISS SLASH", g_sound_ghost_miss_slash)
	}
	if (ArraySize(g_sound_ghost_miss_wall) == 0)
	{
		for (index = 0; index < sizeof sound_ghost_miss_wall; index++)
			ArrayPushString(g_sound_ghost_miss_wall, sound_ghost_miss_wall[index])
		
		// Save to external file
		amx_save_setting_string_arr(ZP_GHOST_SETTINGS_FILE, "Sounds", "GHOST MISS WALL", g_sound_ghost_miss_wall)
	}
	if (ArraySize(g_sound_ghost_hit_normal) == 0)
	{
		for (index = 0; index < sizeof sound_ghost_hit_normal; index++)
			ArrayPushString(g_sound_ghost_hit_normal, sound_ghost_hit_normal[index])
		
		// Save to external file
		amx_save_setting_string_arr(ZP_GHOST_SETTINGS_FILE, "Sounds", "GHOST HIT NORMAL", g_sound_ghost_hit_normal)
	}
	if (ArraySize(g_sound_ghost_hit_stab) == 0)
	{
		for (index = 0; index < sizeof sound_ghost_hit_stab; index++)
			ArrayPushString(g_sound_ghost_hit_stab, sound_ghost_hit_stab[index])
		
		// Save to external file
		amx_save_setting_string_arr(ZP_GHOST_SETTINGS_FILE, "Sounds", "GHOST HIT STAB", g_sound_ghost_hit_stab)
	}
	if (ArraySize(g_sound_ghost_idle) == 0)
	{
		for (index = 0; index < sizeof sound_ghost_idle; index++)
			ArrayPushString(g_sound_ghost_idle, sound_ghost_idle[index])
		
		// Save to external file
		amx_save_setting_string_arr(ZP_GHOST_SETTINGS_FILE, "Sounds", "GHOST IDLE", g_sound_ghost_idle)
	}
	if (ArraySize(g_sound_ghost_idle_last) == 0)
	{
		for (index = 0; index < sizeof sound_ghost_idle_last; index++)
			ArrayPushString(g_sound_ghost_idle_last, sound_ghost_idle_last[index])
		
		// Save to external file
		amx_save_setting_string_arr(ZP_GHOST_SETTINGS_FILE, "Sounds", "GHOST IDLE LAST", g_sound_ghost_idle_last)
	}
	...
	{
		for (index = 0; index < ArraySize(g_sound_ghost_pain); index++)
		{
			ArrayGetString(g_sound_ghost_pain, index, sound, charsmax(sound))
			precache_sound(sound)
		}
		for (index = 0; index < ArraySize(g_sound_ghost_die); index++)
		{
			ArrayGetString(g_sound_ghost_die, index, sound, charsmax(sound))
			precache_sound(sound)
		}
		for (index = 0; index < ArraySize(g_sound_ghost_fall); index++)
		{
			ArrayGetString(g_sound_ghost_fall, index, sound, charsmax(sound))
			precache_sound(sound)
		}
		for (index = 0; index < ArraySize(g_sound_ghost_miss_slash); index++)
		{
			ArrayGetString(g_sound_ghost_miss_slash, index, sound, charsmax(sound))
			precache_sound(sound)
		}
		for (index = 0; index < ArraySize(g_sound_ghost_miss_wall); index++)
		{
			ArrayGetString(g_sound_ghost_miss_wall, index, sound, charsmax(sound))
			precache_sound(sound)
		}
		for (index = 0; index < ArraySize(g_sound_ghost_hit_normal); index++)
		{
			ArrayGetString(g_sound_ghost_hit_normal, index, sound, charsmax(sound))
			precache_sound(sound)
		}
		for (index = 0; index < ArraySize(g_sound_ghost_hit_stab); index++)
		{
			ArrayGetString(g_sound_ghost_hit_stab, index, sound, charsmax(sound))
			precache_sound(sound)
		}
		for (index = 0; index < ArraySize(g_sound_ghost_idle); index++)
		{
			ArrayGetString(g_sound_ghost_idle, index, sound, charsmax(sound))
			precache_sound(sound)
		}
		for (index = 0; index < ArraySize(g_sound_ghost_idle_last); index++)
		{
			ArrayGetString(g_sound_ghost_idle_last, index, sound, charsmax(sound))
			precache_sound(sound)
		}
	}
...
public fw_EmitSound(id, channel, const sample[], Float:volume, Float:attn, flags, pitch)
{
	...
	if (get_pcvar_num(cvar_zombie_sounds_pain))
	{
		...
		if (sample[7] == 'b' && sample[8] == 'h' && sample[9] == 'i' && sample[10] == 't')
		{
			...
			{
				ArrayGetString(g_sound_ghost_pain, random_num(0, ArraySize(g_sound_ghost_pain) - 1), sound, charsmax(sound))
				emit_sound(id, channel, sound, volume, attn, flags, pitch)
				return FMRES_SUPERCEDE;
			}
		...
		if (sample[7] == 'd' && ((sample[8] == 'i' && sample[9] == 'e') || (sample[8] == 'e' && sample[9] == 'a')))
		{
			...
			{
				ArrayGetString(g_sound_ghost_die, random_num(0, ArraySize(g_sound_ghost_die) - 1), sound, charsmax(sound))
				emit_sound(id, channel, sound, volume, attn, flags, pitch)
				return FMRES_SUPERCEDE;
			}
		...
		if (sample[10] == 'f' && sample[11] == 'a' && sample[12] == 'l' && sample[13] == 'l')
		{
			...
			if (LibraryExists(LIBRARY_GHOST, LibType_Library) && zp_class_ghost_get(id))
			{
				ArrayGetString(g_sound_ghost_fall, random_num(0, ArraySize(g_sound_ghost_fall) - 1), sound, charsmax(sound))
				emit_sound(id, channel, sound, volume, attn, flags, pitch)
				return FMRES_SUPERCEDE;
			}
	...
	if (get_pcvar_num(cvar_zombie_sounds_attack))
	{
		...
		if (sample[8] == 'k' && sample[9] == 'n' && sample[10] == 'i')
		{
			if (sample[14] == 's' && sample[15] == 'l' && sample[16] == 'a') // slash
			{
				...
				if (LibraryExists(LIBRARY_GHOST, LibType_Library) && zp_class_ghost_get(id))
				{
					ArrayGetString(g_sound_ghost_miss_slash, random_num(0, ArraySize(g_sound_ghost_miss_slash) - 1), sound, charsmax(sound))
					emit_sound(id, channel, sound, volume, attn, flags, pitch)
					return FMRES_SUPERCEDE;
				}
			...
			if (sample[14] == 'h' && sample[15] == 'i' && sample[16] == 't') // hit
			{
				if (sample[17] == 'w') // wall
				{
					...
					if (LibraryExists(LIBRARY_GHOST, LibType_Library) && zp_class_ghost_get(id))
					{
						ArrayGetString(g_sound_ghost_miss_wall, random_num(0, ArraySize(g_sound_ghost_miss_wall) - 1), sound, charsmax(sound))
						emit_sound(id, channel, sound, volume, attn, flags, pitch)
						return FMRES_SUPERCEDE;
					}
				...
				else
				{
					...
					if (LibraryExists(LIBRARY_GHOST, LibType_Library) && zp_class_ghost_get(id))
					{
						ArrayGetString(g_sound_ghost_hit_normal, random_num(0, ArraySize(g_sound_ghost_hit_normal) - 1), sound, charsmax(sound))
						emit_sound(id, channel, sound, volume, attn, flags, pitch)
						return FMRES_SUPERCEDE;
					}
			...
			if (sample[14] == 's' && sample[15] == 't' && sample[16] == 'a') // stab
			{
				...
				if (LibraryExists(LIBRARY_GHOST, LibType_Library) && zp_class_ghost_get(id))
				{
					ArrayGetString(g_sound_ghost_hit_stab, random_num(0, ArraySize(g_sound_ghost_hit_stab) - 1), sound, charsmax(sound))
					emit_sound(id, channel, sound, volume, attn, flags, pitch)
					return FMRES_SUPERCEDE;
				}
...
public zombie_idle_sounds(taskid)
{
	...
	{
		// Ghost Class loaded?
		if (LibraryExists(LIBRARY_GHOST, LibType_Library) && zp_class_ghost_get(ID_IDLE_SOUNDS))
		{
			ArrayGetString(g_sound_ghost_idle_last, random_num(0, ArraySize(g_sound_ghost_idle_last) - 1), sound, charsmax(sound))
	...
	else
	{
		...
		if (LibraryExists(LIBRARY_GHOST, LibType_Library) && zp_class_ghost_get(ID_IDLE_SOUNDS))
		{
			ArrayGetString(g_sound_ghost_idle, random_num(0, ArraySize(g_sound_ghost_idle) - 1), sound, charsmax(sound))
			emit_sound(ID_IDLE_SOUNDS, CHAN_VOICE, sound, 1.0, ATTN_NORM, 0, PITCH_NORM)
			return;
		}
